...
function setMainClass {
  main_class="com.usoft.framework.launcher.LauncherApplication"
}
...
function runJavaService {
  ...
  java -Dspring.profiles.active="$profile" $command 2>&1
}

function start() {
  local_app_name=$1
  echo "Starting ${local_app_name}"
  pid=$(ps aux | grep ${local_app_name} | grep -v grep | awk '{print $2}' || true)
  if [[ "$pid" == "" ]]; then
